...
| `/check <REQ-id>` / "测试并审查" | check | §2.5 |
| `/deliver <REQ-id>` / "交付上线" / "归档 REQ-xxx" | deliver | §2.6 |
| `/prototype <REQ-id>` / "生成原型" | prototype | §2.7 |
...
- 路由：XS/S → /code；M/L/XL → /design；伪需求 🔴 → 修复后重跑
