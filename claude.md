...
#### Step 3：根据工作量确定输出物规范 + 自动原型判定

根据最终等级，确定该需求需要什么规格的输出物，同时**自动判定是否需要生成UI原型**。

**【自动原型判定】**

```
【Step 3A】原型必需性判定（自动执行）
   ├─ 分析4个维度：功能类型、复杂度、涉众、验收标准
   ├─ 计算总分（0-10分）
   └─ 判定原型需求等级
   
   ├─ 分数 ≤2  → 不需要原型
   ├─ 分数 3-4 → 输出低保真草图（ASCII线框图）
   ├─ 分数 5-7 → 输出HTML可交互原型
   └─ 分数 ≥8  → 输出HTML完整原型 + Figma设计稿

【Step 3B】原型生成（若需要）
   ├─ L1原型（低保真）：5分钟生成 → Markdown线框图
   ├─ L2原型（标准）：10分钟生成 → HTML可交互（Ant Design）
   └─ L3原型（高保真）：20分钟生成 → HTML完整 + Figma设计
   
   输出：docs/prototype/REQ-XXXXXXXX.html 或 .figma 或 -wireframe.md
```
...
|---|---|---|---|
| 纯后端API | 0分 | 不需要 | 无UI |
| 简单登录表单 | 2分 | 不需要 | 标准业务流程，描述充分 |
| 权限管理系统 + 多页面 | 5分 | HTML原型 | 功能复杂，需展示流程 |
| 数据大屏 + 用户反馈 | 9分 | HTML+Figma | 高保真，涉及多个涉众 |
...
- 原型：强烈推荐（若判定分数≥5，自动生成HTML或Figma）
- 检查方式：AI初检 + 人工终审

**XL等级（超大型，需拆分）**
- 必填字段：L的全部 + iteration_plan + milestones
- 原型：必须生成（若判定分数≥8，自动生成HTML+Figma混合）
- 检查方式：AI初检 + 人工终审 + 团队评审
...
【关键：原型不增加工作量评估】
   → 原型只是"沟通工具"，最终代码实现才是真实工作
   → 原型花费的10-20分钟，节省的是5-10天的返工
```
