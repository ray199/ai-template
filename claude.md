...
2. 【Step 0B】输入质量识别（2-3秒）
   ✅ T1（一句话）→ 推荐3问引导
   ✅ T2（零散文字 / 原型截图）→ 推荐推断+澄清
   ✅ T3（半结构化）→ 推荐补全+确认
   ✅ T4（完整PRD）→ 直接结构化
   → 输出：质量等级 + 推荐策略
   
   **截图/原型图输入说明**：可直接粘贴原型截图、UI设计图、手绘草图。
   AI 从截图中识别：页面数量、功能区域、表单字段、交互按钮、业务流程，按 T2 处理。
```

**Step 1执行方式**（根据质量采用对应策略）

- **T1用户体验**：看到3个关键问题 → 回答 → 自动生成完整文档（3-5分钟）
- **T2用户体验**（含截图）：看到推断内容 + 3个澄清问题 → 确认+回答 → 自动生成文档（5-10分钟）
...
> 兜底规则：单个3分项且涉及核心业务 → 升L；多个3分项或跨迭代 → 升XL；引入 AI/LLM 功能 → 最低升L；前端新增5个以上页面或复杂交互（如可视化大屏、拖拽编辑器）→ 最低升L
...
| `/design REQ-xxx` | 技术设计（M/L/XL） | 架构 + DB + 接口设计 + 前端UI设计（若含前端） | `/code` |
| `/code REQ-xxx` | 编码实现 | 后端代码 + 前端页面/组件/API层（若含前端） + 测试骨架 | `/check` |
| `/check REQ-xxx` | 测试+审查 | 后端接口测试 + 前端人工验收清单 + 5维度代码审查 | AI 告知 |
| `/deliver REQ-xxx` | 交付上线 | 上线前检查（含前端构建/部署） + 文档归档 | 完成 |
| `/prototype REQ-xxx` | 原型（按需） | 手动触发原型生成或重新生成；`/intake` 中已自动判定并生成 | — |
...
- `backlog/REQ-xxx.md` - 待开发的需求（`/intake` 写入）
- `done/REQ-xxx/` - 已交付的需求（`/deliver` 归档，文件保持原名，包含该需求全生命周期文档）

**设计文档**：`docs/design/`
- `REQ-XXX-design.md` - 技术设计文档
- `REQ-XXX-code-report.md` - 编码完成报告（与设计文档同目录，便于追踪「设计→实现」对应关系）

**测试文档**：`docs/test/`
- `REQ-XXX-test.md` - 测试报告（含单元测试、集成测试结论）
