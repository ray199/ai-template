...
1. 【Step 0】版本上下文扫描 - 检测项目技术栈（pom.xml / package.json 等），确定前后端版本
   - 若 pom.xml 和 src/ 均不存在 → **全新项目**：先读取设计文档中"项目骨架规划"节，生成骨架文件（pom.xml、Application.java、application.yml、基础包结构 + 前端骨架），验证可启动后再继续生成业务代码；**根包名必须使用 `com.pangeo.{项目名}`**
   - 若已有项目 → 扫描现有 .java 文件的 `package` 声明，沿用原有包结构，不得更改
