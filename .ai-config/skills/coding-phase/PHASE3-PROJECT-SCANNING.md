...
### 第四步：确认工作量
1. 工作量评估已内置于 `/intake`（Step 2 自动执行，无需单独命令）
2. `/intake` 结束时会明确告知等级和下一步指令
...
| 阶段1 | `/intake` | 需求接入（结构化 + 工作量评估 + 伪需求扫描，一步完成） |
| 阶段2 | `/design` | 技术设计（M/L/XL 等级必需，S 跳过） |
| 阶段2 | `/code` | 编码实现 |
| 阶段3 | `/check` | 测试+代码审查（一步完成） |
| 阶段4 | `/deliver` | 交付上线 |
